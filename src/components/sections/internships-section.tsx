                  ...
                  <div className="md:w-1/2 md:pr-8">
                    {index % 2 === 0 ? (
                      <div className="p-6 bg-background rounded-xl border border-border shadow-md hover:shadow-primary/20 hover:border-primary/50 transition-all duration-300 hover:-translate-y-1">
                        <InternshipCardContent internship={internship} formatDateRange={formatDateRange} onSelect={() => setSelectedInternship(internship)} />
                      </div>
                    ) : <div></div>}
                  </div>
...
const InternshipCardContent = ({ internship, formatDateRange, onSelect }: { internship: Internship; formatDateRange: (start: string, end: string) => string; onSelect: () => void; }) => (
  <>
    ...
    <Button variant="link" onClick={onSelect} className="p-0 h-auto mt-4 text-sm">
